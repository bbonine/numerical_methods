/* 
********************************************************
File Name: leapfrog.cpp 
Description: Numerically Integrate
    d^2/dx^2 = -w^2 x
For w = m = k = 1 using the Leapfrog algorithm
Date: 03/09/21
Author: Brett Bonine
******************************************************
*/
...
double x_old,x_new,v_old,v_new,a_old,a_new,t,E;

// Main Program
int main(){
    cout <<"--------------------------------------------------------------------------------------- \n";
    cout <<"leapfrog.cpp: Solve a SHO via the leapfrog Integration Method. \n ";
    cout <<"--------------------------------------------------------------------------------------- \n";
    ...
    // Begin integration:
    ofstream myfile;
	myfile.open("../outputs/leapfrog.txt");
    ...
    for (int i = 1; i < steps; i++){
        t = t + dt;
        a_old = -x_old;        // Acceleration at current step.
        x_new = x_old + (v_old*dt)+ (0.5*(a_old*(pow(dt,2.)))); // Use to find new position after dt

        a_new = -x_new; // Acceleration at new position
        v_new = v_old + (0.5*(a_old+a_new)*dt); // Find velocity at new step

        E = energy(x_new,v_new);
    ...
    myfile.close();
    cout << "Integration Complete! See ouutput file for results.";
